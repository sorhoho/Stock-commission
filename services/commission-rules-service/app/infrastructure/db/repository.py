...
class AgreementSpecRepository:
    ...
    async def get_by_id(self, spec_id: str) -> AgreementSpec | None:
        result = await self._session.execute(select(AgreementSpec).where(AgreementSpec.id == spec_id, ~AgreementSpec.is_deleted))
    ...
    async def update(self, spec_id: str, updates: dict) -> AgreementSpec | None:
        await self._session.execute(update(AgreementSpec).where(AgreementSpec.id == spec_id).values(**updates))
        return await self.get_by_id(spec_id)

    async def soft_delete(self, spec_id: str) -> None:
        await self._session.execute(update(AgreementSpec).where(AgreementSpec.id == spec_id).values(is_deleted=True))
...
class AgreementRepository:
    ...
    async def get_by_id(self, agreement_id: str) -> Agreement | None:
        result = await self._session.execute(select(Agreement).where(Agreement.id == agreement_id))
        return result.scalar_one_or_none()

    async def get_active_for_party(self, party_id: str, tenant_id: str) -> Agreement | None:
        result = await self._session.execute(
            select(Agreement).where(
                Agreement.party_id == party_id,
    ...
    async def list_with_filters(self, tenant_id: str, party_id: str | None = None, status: str | None = None) -> list[Agreement]:
        ...
        if party_id:
            q = q.where(Agreement.party_id == party_id)
        if status:
    ...
    async def update_status(self, agreement_id: str, status: str) -> None:
        await self._session.execute(update(Agreement).where(Agreement.id == agreement_id).values(status=status))
...
class CommissionRuleRepository:
    ...
    async def get_by_id(self, rule_id: str) -> CommissionRule | None:
        result = await self._session.execute(select(CommissionRule).where(CommissionRule.id == rule_id, ~CommissionRule.is_deleted))
        return result.scalar_one_or_none()

    async def list_for_spec(self, agreement_spec_id: str) -> list[CommissionRule]:
        result = await self._session.execute(
            select(CommissionRule).where(
                CommissionRule.agreement_spec_id == agreement_spec_id,
    ...
    async def soft_delete(self, rule_id: str) -> None:
        await self._session.execute(update(CommissionRule).where(CommissionRule.id == rule_id).values(is_deleted=True))
